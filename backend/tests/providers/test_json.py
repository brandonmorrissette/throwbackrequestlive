...
def test_given_time_when_app_json_dumps_then_return_null(app: Flask) -> None:
    test_time = time(12, 34, 56)
    result = app.json.dumps(test_time)
    assert result == "null"


def test_given_time_in_dict_when_app_json_dumps_then_return_null_value_in_dict(
    app: Flask,
) -> None:
...
def test_given_supported_type_when_app_json_dumps_then_return_value(app: Flask) -> None:
    test_dict = {"key": "value"}
    result = app.json.dumps(test_dict)

from decorators.auth import admin_required
from flask import Blueprint
from flask import current_app as app
from flask import jsonify, request
from services.rds_service import RDSService

data_bp = Blueprint("data", __name__)
rds_service = RDSService()


@data_bp.route("/tables", methods=["GET"])
@admin_required
def list_tables():
    try:
        tables = rds_service.list_tables()
        return jsonify(list(tables)), 200
    except Exception as e:
        app.logger.error(f"Error listing tables: {e}")
        return jsonify({"error": str(e)}), 500


@data_bp.route("/tables/<table_name>/schema", methods=["GET"])
@admin_required
def get_table_schema(table_name):
    try:
        rds_service.validate_table_name(table_name)
        schema = rds_service.get_table_schema(table_name)
        return jsonify(schema), 200
    except Exception as e:
        app.logger.error(f"Error getting table schema: {e}")
        return jsonify({"error": str(e)}), 500


@data_bp.route("/tables/<table_name>", methods=["GET"])
@admin_required
def fetch_rows(table_name):
    filters = request.args.get("filters")
    limit = request.args.get("limit")
    offset = request.args.get("offset")
    sort_by = request.args.get("sort_by")
    sort_order = request.args.get("sort_order", "asc")
    try:
        rds_service.validate_table_name(table_name)
        rows = rds_service.read_rows(
            table_name, filters, limit, offset, sort_by, sort_order
        )
        return jsonify(rows), 200
    except Exception as e:
        app.logger.error(f"Error fetching rows: {e}")
        return jsonify({"error": str(e)}), 500


@data_bp.route("/tables/<table_name>", methods=["POST"])
@admin_required
def insert_row(table_name):
    data = request.get_json()
    try:
        rds_service.validate_table_name(table_name)
        rds_service.validate_columns(table_name, data.keys())
        rds_service.create_row(table_name, data)
        return jsonify({"message": "Row inserted successfully."}), 201
    except Exception as e:
        app.logger.error(f"Error inserting row: {e}")
        return jsonify({"error": str(e)}), 500


@data_bp.route("/tables/<table_name>/<row_id>", methods=["PUT"])
@admin_required
def update_row(table_name, row_id):
    data = request.get_json()
    try:
        rds_service.validate_table_name(table_name)
        rds_service.validate_columns(table_name, data.keys())
        rds_service.update_row(table_name, row_id, data)
        return jsonify({"message": "Row updated successfully."}), 200
    except Exception as e:
        app.logger.error(f"Error updating row: {e}")
        return jsonify({"error": str(e)}), 500


@data_bp.route("/tables/<table_name>/<row_id>", methods=["DELETE"])
@admin_required
def delete_row(table_name, row_id):
    try:
        rds_service.validate_table_name(table_name)
        rds_service.delete_row(table_name, row_id)
        return jsonify({"message": "Row deleted successfully."}), 200
    except Exception as e:
        app.logger.error(f"Error deleting row: {e}")
        return jsonify({"error": str(e)}), 500


@data_bp.route("/query", methods=["POST"])
@admin_required
def execute_query():
    data = request.get_json()
    query = data.get("query")
    params = data.get("params", {})
    try:
        result = rds_service.execute_custom_query(query, params)
        return jsonify(result), 200
    except Exception as e:
        app.logger.error(f"Error executing query: {e}")
        return jsonify({"error": str(e)}), 500
